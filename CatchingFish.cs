...
namespace CatchingFish
{
    ...
    public class fishCatching : Microsoft.Xna.Framework.Game
    {
        ...
        public fishCatching(GraphicsDeviceManager gdm)
        {
        ...
        /// 用于加载所有内容。
        /// </summary>
        public void loadContent(ContentManager cm, SpriteBatch sb, GraphicsDeviceManager gdm)
        {
            // 创建新的 SpriteBatch，可将其用于绘制纹理。
            scoreFont = cm.Load<SpriteFont>("Times New Roman");
            logo = cm.Load<Texture2D>("cat_net");
            Viewport viewport = gdm.GraphicsDevice.Viewport;
            logoPosition = new Vector2((viewport.Width - logo.Width)/2, viewport.Height - logo.Height);
			
            for (int i = 0; i < 8; i++)
            {
                fishes[i].fish_pic = cm.Load<Texture2D>("fish");
            }
            ...
            pause = cm.Load<Texture2D>("pause");
            play = cm.Load<Texture2D>("play");
        ...
        public void draw(GameTime gameTime, SpriteBatch sb)
        {
            ...
            sb.Draw(logo, logoPosition, Color.White);
            for (int i = 0; i < 8; i++)
            {
                if (!fishes[i].isCaught)
                {
                    sb.Draw(fishes[i].fish_pic, fishes[i].position, fishes[i].color);
                }
                /*else
				 {
            ...
            sb.DrawString(scoreFont, "Fishes caught: " + fishCount, new Vector2(10, 10), Color.Yellow, 0, Vector2.Zero, 1, SpriteEffects.None, 1);
        }
