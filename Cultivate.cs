...
namespace WindowsPhoneGame
{
    ...
    /// 这是游戏的主类型
    /// </summary>
    public class Cultivate : Microsoft.Xna.Framework.Game
    {
        ...
        public Cultivate(GraphicsDeviceManager gdm)
        {
        ...
        /// 用于加载所有内容。
        /// </summary>
        protected void loadContent(ContentManager cm)
        {
            backgroundTexture = cm.Load<Texture2D>("background");
			
            storageFont = cm.Load<SpriteFont>("Times New Roman");
			
            
			
            for (int i = 0; i < 6; i++)
            {
                field[i].place = cm.Load<Texture2D>("solid");
            }
			
            solid = cm.Load<Texture2D>("solid");
            seed_carrot = cm.Load<Texture2D>("seed_carrot");
            seed_Chinese_cabbage = cm.Load<Texture2D>("seed_chinese_cabbage");
            fruit_carrot = cm.Load<Texture2D>("fruit_carrot");
            fruit_cabbage = cm.Load<Texture2D>("fruit_cabbage");
			
            //显示田地里的种子图片
            seed_cabbage_big = cm.Load<Texture2D>("seed_Chinese_cabbage_big");
            seed_carrot_big = cm.Load<Texture2D>("seed_carrot_big");
			
            harvest = cm.Load<Texture2D>("harvest");
            backwards = cm.Load<Texture2D>("backwards");
			
            soundEffect = cm.Load<SoundEffect>("Windows Ding");
        }
        ...
        /// </summary>
        /// <param name="gameTime">提供计时值的快照。</param>
        protected void draw(GameTime gameTime, SpriteBatch sb)
        {
            ...
            //添加背景图片
            sb.Draw(backgroundTexture, new Rectangle(0, 0, Window.ClientBounds.Width, Window.ClientBounds.Height), null, Color.White, 0, Vector2.Zero, SpriteEffects.None, 0);
			
            //添加返回按钮
            sb.Draw(backwards, new Vector2(10, 20), Color.White);
			
            //添加数目栏
            sb.DrawString(storageFont, "Chinese_cabbage: " + Chinese_cabbage, new Vector2(80, 20), Color.Yellow, 0, Vector2.Zero, 1, SpriteEffects.None, 1);
            sb.DrawString(storageFont, "Carrot: " + carrot, new Vector2(250, 20), Color.Yellow, 0, Vector2.Zero, 1, SpriteEffects.None, 1);
			
            //添加收割按钮
            sb.Draw(harvest, new Vector2(420, 6), Color.White);
            ...
            for (int j = 0; j < 6; j++)
            {
                if (field[j].isSaw == 0)
                {//如果未播种
                    sb.Draw(solid, field[j].position, field[j].color);
                }
                else
                {
                    if (field[j].isSaw == 1)
                    {//如果播种了
                        ...
                        {//如果播种了但没有成熟
                            if (field[j].type == 1)
                            {
                                sb.Draw(seed_cabbage_big, field[j].position, field[j].color);
                            }
							
                            if (field[j].type == 2)
                            {
                                sb.Draw(seed_carrot_big, field[j].position, field[j].color);
                            }
                            if (field[j].type == 0)
                            {
                                sb.Draw(solid, field[j].position, field[j].color);
                        ...
                        else if (field[j].isRipe == 1)
                        {
                            if (field[j].type == 1)
                            {
                                sb.Draw(fruit_cabbage, field[j].position, field[j].color);
                            }
                            if (field[j].type == 2)
                            {
                                sb.Draw(fruit_carrot, field[j].position, field[j].color);
                            }
                        }
                    }
                }
            ...
            if (flat_01 == 1)
            {
                sb.Draw(seed_Chinese_cabbage, position_01, Color.Yellow);
            }
            else
            {
            ...
            if (flat_02 == 1)
            {
                sb.Draw(seed_carrot, position_02, Color.Yellow);
            }
            else
            {
        ...
        {//注意，在任何时候只要退出游戏都要调用这个方法，这两个参数由调用者获取并传递
            //保存游戏状态信息，这里包括每块田的信息和收获的白数书和胡萝卜数
			#if WINDOWS_PHONE
				IsolatedStorageFile savegameStorage = IsolatedStorageFile.GetUserStoreForApplication();
#else
